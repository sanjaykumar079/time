...
# Updated TEACHERS_BY_FIELD with predefined teachers
TEACHERS_BY_FIELD = {
    "CSE": [
...
# Corrected SECTION_COUNTS definition
SECTION_COUNTS = {(field["field_id"], str(year["year"])): year["num_sections"] for field in FIELDS for year in YEARS}

# Pre-booked slots for each section: (field_id, year, section_letter) -> [(day, period), ...]
PRE_BOOKED_SLOTS = {
    ("CSE", "2", "A"): [],
    ("CSE", "2", "B"): [],
    ("AIML", "2", "A"): [],
...
class Timetable:
    def __init__(self, subjects, sections, field_id, year):
        ...
        self.year = year
        self.pre_booked_slots = {}  # (day, period, section_id) -> True
    ...
    def reserve_pre_booked_slots(self):
        for section in self.sections:
            ...
            pre_booked = PRE_BOOKED_SLOTS.get(pre_booked_key, [])

            for day, period in pre_booked:
                self.pre_booked_slots[(day, period, section_id)] = True
                print(f"Pre-booked slot for {section_id} at {day} Period {period}")
    ...
    def is_teacher_available(self, day, period, faculty, section_id):
        ...
        # Check for consecutive periods (both previous and next period)
        ...
        # Check max 2 slots per day for this section
    ...
    def random_init(self):
        # Reserve pre-booked slots
        self.reserve_pre_booked_slots()

        # Initialize slot tracking for each teacher
            ...
            for day, period in all_slots:
                ...
                for i in range(len(subject_list)):
                    ...
                    if self.is_teacher_available(day, period, faculty, section_id):
                        # Check if teacher has remaining slots to assign
                        ...
                        if current_slots >= required_slots:
                            continue  # Skip if teacher has already been assigned all required slots
        ...
        for section_id in self.subjects:
            for teacher_id, _, _, slots_needed in self.subjects[section_id]:
                ...
                if assigned_slots != slots_needed:
                    print(f"Warning: Teacher {teacher_id} in section {section_id} was assigned {assigned_slots}/{slots_needed} slots.")
    ...
    def calculate_fitness(self):
        ...
        # Check teacher overlaps, room conflicts, consecutive periods, and daily limits
...
def select_options():
    if request.method == 'POST':
        ...
        if action == 'pre_book_slots':
            ...
            PRE_BOOKED_SLOTS[(field_id, year_str, section_letter)] = pre_booked_slots
            flash(f"Pre-booked slots updated for {field_id} - Year {year_str} - Section {section_letter}.")
            return redirect(url_for('select_options', field=field_id, year=year_str))
            ...
            for section in sections:
                ...
                for day, period in pre_booked:
                    ...
                    if period not in pre_booked_data[day]:
                        pre_booked_data[day][period] = {}
                    pre_booked_data[day][period][section_id] = True
        ...
        if action in ['add_teacher', 'edit_teacher', 'delete_teacher']:
            ...
            if action == 'add_teacher':
                ...
                # Calculate total assigned slots for the section
                pre_booked_key = (field_id, year_str, section)
                pre_booked_slots = len(PRE_BOOKED_SLOTS.get(pre_booked_key, []))
        ...
        try:
            ...
            if year_data:
                ...
                for i in range(num_sections):
                    ...
                    # Calculate pre-booked slot counts
                    pre_booked_key = (field_id, year_id, section_letter)
                    pre_booked_slots = PRE_BOOKED_SLOTS.get(pre_booked_key, [])
                    pre_booked_counts[section_letter] = len(pre_booked_slots)
                    # Calculate assigned slots for teachers
